...
def run_tests():
    ...
    for file in files:
        ...
        try:
            ...
            global graph, itemset, Vmax, Vmin, W, R, v_w
            ...
            print(f"Parametry problemu:")
            print(f"Vmax: {Vmax}, Vmin: {Vmin}")
            print(f"Pojemność plecaka (W): {W}")
            print(f"Współczynnik wynajmu (R): {R}")
            print(f"Współczynnik spadku prędkości (v_w): {v_w}")
            
            for params_name, params in [("dobre", good_parameters), ("słabe", weak_parameters)]:
                print(f"\nTestowanie z parametrami: {params_name}")
                pso = PSO(**params)
            ...
            for run in range(5):
                print(f"\nUruchomienie {run + 1}/5 dla pliku {file}")
                print(f"Parametry problemu: Vmax={Vmax}, Vmin={Vmin}, W={W}, R={R}")
                ...
                stability_run_results.append({
                    ...
                    'Zysk z przedmiotów': total_profit,
                    'Waga przedmiotów': total_weight,
                    'Parametry problemu': f"Vmax={Vmax}, Vmin={Vmin}, W={W}, R={R}"
                })
